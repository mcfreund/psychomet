---
title: "Trial-level analyses: multivariate projections --- within-task (leave-one-run-out)"
output:
  html_document:
    toc: true
    toc_depth: 3
...
fnames <- c(
  ...
  network_hilo = 
    "projs-trials_axis-hilo_cval-wthntsk_schaefer-network07_prew-none_stand-axes_center-pattern_resid-errts.RDS",
  network_hilo_prw = 
    "projs-trials_axis-hilo_cval-wthntsk_schaefer-network07_prew-concat-runs-multi-alltask_stand-axes_center-pattern_resid-errts.RDS",
  parcel_hilo = 
    "projs-trials_axis-hilo_cval-wthntsk_schaefer-parcel_prew-none_stand-axes_center-pattern_resid-errts.RDS",
  parcel_hilo_prw = 
    "projs-trials_axis-hilo_cval-wthntsk_schaefer-parcel_prew-concat-runs-multi-alltask_stand-axes_center-pattern_resid-errts.RDS",
  
  network_ons = 
    "projs-trials_axis-ons_cval-wthntsk_schaefer-network07_prew-none_stand-axes_center-none_resid-errts.RDS",
  network_ons_prw = 
    "projs-trials_axis-ons_cval-wthntsk_schaefer-network07_prew-concat-runs-multi-alltask_stand-axes_center-none_resid-errts.RDS",
  parcel_ons = 
    "projs-trials_axis-ons_cval-wthntsk_schaefer-parcel_prew-none_stand-axes_center-none_resid-errts.RDS",
  parcel_ons_prw = 
    "projs-trials_axis-ons_cval-wthntsk_schaefer-parcel_prew-concat-runs-multi-alltask_stand-axes_center-none_resid-errts.RDS"
  
)
fnames <- setNames(file.path(here("out", "icc"), fnames), names(fnames))

cols <- c("subj", "task", "run", "trial.num", "trial.type", "acc")
trialinfo <- behav %>% map(~ select(.x, all_of(cols))) %>% rbindlist
trialinfo <- trialinfo %>% mutate(run = as.character(run))  ## for joining with projections
...
get_hilo <- function(d, .by = c("subj", "task", "roi", "hilo")) {

  ## get hilo per task:

  d <- d[!is.na(value)]
  d$hilo <- as.character(NA)
  d[trialtype %in% hi]$hilo <- "hi"
  d[trialtype %in% lo]$hilo <- "lo"

  d_subj <-
    d[hilo %in% c("hi", "lo"), .(value = mean(value)), by = .by]  ## aggregate
  d_subj <- dcast(d_subj, ... ~ hilo)  ## to wide
...
d_subjrun_parc_hilo <- get_trial_data("parcel_hilo") %>% get_hilo(.by = c("subj", "task", "roi", "run", "hilo"))
d_subjrun_parc_hilo_w <- d_subjrun_parc_hilo %>% 
  select(-hi, -lo) %>% 
  dcast(... ~ run, value.var = "hi_v_lo", prefix = "run_")

d_r_parc_hilo <- d_subjrun_parc_hilo_w[, .(r = cor(`1`, `2`)), by = c("task", "roi")]

# d_r_parc_hilo %>% arrange(-r)



d_subjrun_parc_hilo_prw <- get_trial_data("parcel_hilo_prw") %>% get_hilo(.by = c("subj", "task", "roi", "run", "hilo"))
d_subjrun_parc_hilo_prw_w <- d_subjrun_parc_hilo_prw %>% 
  select(-hi, -lo) %>% 
  dcast(... ~ run, value.var = "hi_v_lo", prefix = "run_")

d_r_parc_hilo_prw <- d_subjrun_parc_hilo_prw_w[, .(r = cor(`1`, `2`)), by = c("task", "roi")]

# d_r_parc_hilo_prw %>% arrange(-r)

d_r_parc_hilo <- merge(d_r_parc_hilo, d_r_parc_hilo_prw, by = c("task", "roi"), suffix = c("_van", "_prw"))


d_r_parc_hilo %>% 
  
  ggplot(aes(r_van, r_prw, color = ifelse(roi %in% key_schaefer$parcel[schaefermd], TRUE, FALSE))) +
...
d_r_parc_hilo %>% arrange(-r_van) %>% View
d_r_parc_hilo %>% arrange(-r_prw)
