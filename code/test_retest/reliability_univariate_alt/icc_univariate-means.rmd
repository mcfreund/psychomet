...
d$hilo <- NULL  ## remove hilo col and rename hilo_all to hilo
d <- rename(d, hilo = hilo_all)

# d$load <- as.numeric(ifelse(d$hilo %in% as.character(2:8), d$hilo, NA))  ##  make stern df
# d[!hilo %in% c("hi", "lo")]$hilo <- NA
# stern <- d[task == "Stern"]
...
ggsave(
  here("out", "test_retest", "figs", "hilo-test-retest_summary-stats_hilo-all.pdf"), 
  p_tr_sum, 
...
ggsave(
  here("out", "test_retest", "figs", "hilo-test-retest_summary-stats_scatter_hilo-all.pdf"), 
  p_tr_sum_scatter, 
...
ggsave(
  here("out", "test_retest", "figs", "hilo-test-retest_lme-gang_dist_hilo-all.pdf"), 
  p_tr_lme_dist, 
...
fnames_results_hbm <- here("out", "test_retest", paste0("fits_hba_chenstyle_", tasks, "_hilo-all.RDS"))
...
for (task_i in seq_along(tasks)) {
    ...
    fname_results <- here("out", "test_retest", paste0("fits_hba_chenstyle_", task_val, "_hilo-all.RDS"))
...
p_posteriors <- posterior_trr_d[roi %in% rois[core32]] %>%
  ...
  # ggplot(aes(cor_subj__mean_wave1__mean_wave2, fill = task, color = task)) +
  ...
  # labs(x = "cor(mean_wave1, mean_wave2)", y = "posterior density", title = "test-retest correlations (mean) in core 32")
  labs(x = "cor(hilo_wave1, hilo_wave2)", y = "posterior density", title = "test-retest correlations (hilo) in core 32")

p_posteriors

# ggsave(here("out", "test_retest", "figs", "posterior_trr_core32_means_hilo-all.pdf"), p_posteriors, width = 10, height = 10)
ggsave(here("out", "test_retest", "figs", "posterior_trr_core32_hilo-all.pdf"), p_posteriors, width = 10, height = 10)
...
ggsave(here("out", "test_retest", "figs", "posterior_scatter_hilo-all.pdf"), p_scatter_means, width = 6, height = 6)
...
ggsave(here("out", "test_retest", "figs", "posterior_scatter_map_hilo-all.pdf"), p_scatter_map, width = 6, height = 6)
...
ggsave(here("out", "test_retest", "figs", "variance_ratio_hilo-all.pdf"), p_vardecomp, width = 6, height = 4.5)
...
ggsave(here("out", "test_retest", "figs", "variance_ratio_hilo-all.pdf"), p_ratio, width = 6, height = 4.5)
...
#     function(x) {
#       # x <- as_draws_matrix(x, variable = "cor_subj__hilo_wave1__hilo_wave2")
#       x <- as_draws_matrix(x, variable = "^cor_.*", regex = TRUE)
#       x <- data.table(x)
#       x
#       }
#     )
# 
# posterior_trr_axcpt <- rbindlist(posterior_trr_axcpt, idcol = "roi")
# 
# 
# r_tr_axcpt <- posterior_trr_axcpt[, 
#                 .(
#                   r_hilo_mean = mean(cor_subj__hilo_wave1__hilo_wave2),
#                   r_hilo_map = get_map(cor_subj__hilo_wave1__hilo_wave2),
#                   r_mean_mean = mean(cor_subj__mean_wave1__mean_wave2),
#                   r_mean_map = get_map(cor_subj__mean_wave1__mean_wave2),
#                   r_hilo_sd = sd(cor_subj__hilo_wave1__hilo_wave2),
#                   r_mean_sd = sd(cor_subj__mean_wave1__mean_wave2)
#                   ), 
#                 by = "roi"
#                 ]
# 
# 
# 
# posterior_trr_axcpt_summary <- 
#   lapply(
#     fits_axcpt, 
#     function(x) {
#       r <- VarCorr(x, variable = "cor_subj__hilo_wave1__hilo_wave2")$subj$cor
#       r_mean <- r["mean_wave1", , "mean_wave2"]
#       r_hilo <- r["hilo_wave1", , "hilo_wave2"]
#       r_summary <- as.data.table(rbind(mean = r_mean, hilo = r_hilo), keep.rownames = "variable")
#       r_summary
#     }
#     )
# 
# 
# posterior_trr_axcpt_summary <- rbindlist(posterior_trr_axcpt_summary, idcol = "roi")
# 
# 
# posterior_trr_axcpt_summary %>%
#   ggplot(aes(variable, Estimate)) +
#   # ggplot(aes(variable, Est.Error)) +
#   geom_boxplot(width = 0.1, fill = "grey40")
# 
# posterior_trr_axcpt_summary %>%
#   filter(Q2.5 > 0, variable == "mean")
# 
# r_sum %>% filter(roi == " LH_Cont_PFCv_1", task == "Axcpt")
# 
# 
# plot(fits_axcpt$RH_Default_PFCv_4, variable = "cor_.*", regex = TRUE)
# posterior_trr_axcpt_summary %>% filter(roi == "RH_Default_PFCv_4")
# r_tr_axcpt %>% filter(roi == "RH_Default_PFCv_4")
# 
# 
# 
