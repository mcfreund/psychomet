---
title: "intersubject discrimination (ID): mathematical definitions and simulation"
output:
  html_document:
    toc: true
    toc_depth: 3
...
Let $y_{tv}$ be an analogous estimate from fold 2 of the experiment (e.g., run 2 or "retest" phase), for a not-necessarily-different subject $t \in 1, \ldots, S$.
...
\[\bar x_{s} = \sum_{v=1}^{V}x_{sv}\]
\[\bar y_{t} = \sum_{v=1}^{V}y_{tv}\]
\[\mathit{u}_{st} = \frac{1}{V}(\bar x_{s} - \bar y_{t})^2\]
...
for (ii in 1:n_sim) {
  ...
  d_univ$m[ii] <- euclidean2(x, y) / V
  d_univ$u[ii] <- euclidean2(sum(x), sum(y)) / V
...
Now the univariate distance is larger.
...
### multivariate (non-uniform) effect
...
subj_pattern <- scale(rnorm(V), scale = FALSE)  ## no univariate effect

for (ii in 1:n_sim) {

  x <- rnorm(V) + subj_pattern  ## pattern across vertices, but across-voxel mean of 0
  y <- rnorm(V) - subj_pattern  ## opposite pattern
...
Now the multivariate distance is larger.
...
for (ii in 1:n_sim) {
  ...
  ## X is a matrix of b-coefficients. subjects as rows, vertices as columns. from fold 1 (e.g., "test").
  ## Y is also a b-coefficient matrix. organized identically to X, but from a different "fold" (e.g., "retest").
  
  X_bar <- cbind(rowSums(X))  ## across-vertex sum of b-coefficients. one per subj.
...
kernel_d2 <- kde2d(d$m, d$u)
image(kernel_d2, col = r, main = "intersubject discrimination index", xlab = "multivariate", ylab = "univariate")
...
for (ii in 1:n_sim) {
  ...
  subj_means <- matrix(rnorm(S), nrow = V, ncol = S, byrow = TRUE)
  X <- matrix(rnorm(V*S), ncol = S) + subj_means  ## add a pattern to each row
  Y <- matrix(rnorm(V*S), ncol = S) + subj_means
  
  d_univ$m[ii] <- idi(pdist2(t(X), t(Y)) / V)  ## multivariate distance
  d_univ$u[ii] <- idi(pdist2(cbind(colSums(X)), cbind(colSums(Y))) / V)  ## univariate distance
...
for (ii in 1:n_sim) {
  ...
  subj_patterns <- matrix(rnorm(V*S), ncol = S)
  subj_patterns <- scale(subj_patterns, scale = FALSE)
  X <- matrix(rnorm(V*S), ncol = S) + subj_patterns
  Y <- matrix(rnorm(V*S), ncol = S) + subj_patterns
  
  d_multiv$m[ii] <- idi(pdist2(t(X), t(Y)) / V)  ## multivariate distance
  d_multiv$u[ii] <- idi(pdist2(cbind(colSums(X)), cbind(colSums(Y))) / V)  ## univariate distance
